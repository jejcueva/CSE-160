(function initWorldRenderer(globalScope) {
  ...
  function renderWorldScene(renderContext) {
    ...
    const foxRoot = createIdentityMatrix();
    foxRoot.translate(FOX_HOME.worldX, 0.28, FOX_HOME.worldZ);
    foxRoot.rotate(FOX_HOME.yaw, 0, 1, 0);
    foxRoot.scale(1.6, 1.6, 1.6);

    renderFox({
      pose: resolveFoxPose(seconds, storyState.completed),
      rootMatrix: foxRoot,
      drawCube: (matrix, color) => drawCube(matrix, color, -1),
      drawCone: (matrix, color) => drawCone(matrix, color, -1),
      matrixUtils,
    });
